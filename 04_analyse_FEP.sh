...
get_other_values () {
site_dir=$DATA/PyLipID_poses/$1/lipid_interactions/Interaction_CARD/Binding_Sites_CARD
...
fepval=`tail -n+2 $DATA/FEP_data/energies/$1_$2_$3.txt | awk '{sum+=$1} END {print (sum/NR)}'`
fep_error=`tail -n+2 $DATA/FEP_data/energies/$1_$2_$3.txt | awk '{sum+=$1;a[NR]=$1}END{for(i in a)y+=(a[i]-(sum/NR))^2;print sqrt(y/(NR-1))}'`
echo -e "$fepval $fep_error $koff $koff_err $occ 0 " >> $DATA/FEP_data/energies/comparison.txt 
}

mkdir -p $DATA/FEP_data/energies
echo "fep fep_error koff koff_error occ" > $DATA/FEP_data/energies/comparison.txt

for i in `ls $DATA/FEP_data/Data | grep -v -e old -e longer`
do
	fep=$DATA/FEP_data/Data/$i/Data
	if [[ -d $fep ]]
	then
		echo $i
		echo $i > $DATA/FEP_data/energies/$i.txt
		for rep in {1..10}
		do
