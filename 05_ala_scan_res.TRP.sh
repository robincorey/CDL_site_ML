...
# Just TRP
list_prune=`sed -n "/Binding site $3$/,/^$/p" $site_file | tail -n+16 | awk -v var=$cutoff '$6>var' | grep  TRP | awk '{print $1}' | tr -d [A-Z]` 
	...
	cat $coords/Protein.itp | while read ;do 
		if [[ "$REPLY" = *$5* ]] && [[ "$REPLY" = *SC* ]] ;then 
			...
			if  [[ "$resnum" = "$5" ]] ;then
				...
				mass=`echo "$REPLY" | awk '{print $8}'`
				echo "$REPLY" | awk -v var=$mass '{print $1" "$2" "$3" "$4" "$5" "$6" "$7" "var" Dum "0" "var}' >> $data/Protein.itp 
			else
...
cp ${ALA_DIR}_TRP/$site/$res/Protein.itp $1/
...
mkdir -p $1/out_files
mkdir -p $1/EM
mkdir -p ${ALA_DIR}_TRP/Data/${2}_${3}_${5}/
for i in `seq 0 2 30` ; do
	...
	for run in {1..10} ;do
        	if [ ! -f ${ALA_DIR}_TRP/Data/${2}_${3}_${5}/md_${i}_$run.tpr ] ;then
                        gmx grompp -f $FEP/MDPs/md_FEP_${i}.mdp -c $1/EM/em_$i.gro -r $1/EM/em_$i.gro -p $1/topol.top -n $1/sys.ndx -o ${ALA_DIR}_TRP/Data/${2}_${3}_${5}/md_${i}_$run.tpr -maxwarn 3 >& $1/out_files/outgrompp
			if [[ ! -f ${ALA_DIR}_TRP/Data/${2}_${3}_${5}/md_${i}_$run.tpr ]]; then echo grompp failed; exit 0 ; fi
			if [[ ! -f ${ALA_DIR}_TRP/Data/${2}_${3}_${5}/md_${i}_$run.gro ]]; then
 			gmx mdrun -v -deffnm ${ALA_DIR}_TRP/Data/${2}_${3}_${5}/md_${i}_$run -pin on -pinoffset 0 -ntomp 6 -ntmpi 1 -nsteps 600000 >& $data/out_files/outmdrun
			fi
		fi
	done
done       
sed "s/#NAME#/$2_$3_$4_$5/g" $FEP/array_all.sh > ${ALA_DIR}_TRP/Data/${2}_${3}_${5}/array.sh
...
for site in 2QFI_11 2IC8_0 2WSX_22 5JWY_1 3B5D_3 3DHW_10 1PW4_3 4Q65_6 4DJI_3; do
	...
	mkdir -p ${ALA_DIR}_TRP/$site/out_files
	mkdir -p ${ALA_DIR}_TRP/$site/res_fep2
	coords=$DATA/FEP/$pdb/$pose/$rep
	#build_free $ALA_DIR/$site/free $pdb $pose
	get_res ${ALA_DIR}_TRP/$site $pdb $pose 	
	for res in `cat ${ALA_DIR}_TRP/$site/res.txt`; do
		data=${ALA_DIR}_TRP/$site/${res}
